...
import com.sinam7.schematicbuilder.SchematicPlugin;
...
public class Schematic {
     ...
     * @param file file to the schematic
     */
    public Schematic(final SchematicPlugin plugin, final File file) {
        this.plugin = plugin;
        this.file = file;
    ...
    public Collection<Location> pasteSchematic(final Location loc, final Player paster, final int time, final Options... option) {
        ...
        try {
            ...
            // set clipboardholder's origin location to its clipboard origin
            // clipboardHolder.setTransform(new CombinedTransform().combine(new AffineTransform().translate(clipboard.getOrigin())).combine(new AffineTransform().rotateY(yaw)));
...
//                        final Location offsetLoc = loc.clone().subtract(offsetX - widthCentre, offsetY - heightCentre, offsetZ - lengthCentre);
                        final Location offsetLoc = loc.clone().subtract(offsetX - widthCentre, offsetY - height + 2, offsetZ - lengthCentre);
    ...
    public void previewSchematic(Player player) {
        ...
        pm.setBuilding(player.getUniqueId(), this);
        pm.setViewDistance(player.getUniqueId(), 10);
    ...
    public Map<Material, Integer> getSchematicMaterialData() {
        Map<Material, Integer> materialValuesMap = new LinkedHashMap<>();
        ...
        Map<Material, Integer> sorted = new LinkedHashMap<>();
        materialValuesMap.entrySet().stream()
